...
{
    public class TabFPag
    {
        public TABFPAG GetByKey(String key)
        {
            ...
            try
            {
                ...
                TABFPAG obj = (from c in ctx.TABFPAG where c.FPAG == key select c).FirstOrDefault();
        ...
        public TABFPAG ProcessarVazios(TABFPAG obj)
        {
        ...
        public ResultList GetAll(Int32 pag, Int32 itemsByPag)
        {
            ...
            {
                var ctx = new DataGiicNetEntities();
                var obj = (from c in ctx.TABFPAG select c).ToList();
                if (obj.Count > 0)
                {
                    r.Status = true;
                    r.Lista = obj;
        ...
        public ResultList Insert(TABFPAG tab)
        {
            ...
            {
                var ctx = new DataGiicNetEntities();
                TABFPAG obj = GetByKey(tab.FPAG);
                if (obj == null) ctx.TABFPAG.Add(tab);
        ...
        public ResultList Update(TABFPAG tab)
        {
            tab = ProcessarVazios(tab);
            var r = new ResultList();
            r.Status = false;
        ...
        public ResultList Valida(TABFPAG tab)
        {
            var r = new ResultList();
            r.Status = false;
            r.Erros = "";
