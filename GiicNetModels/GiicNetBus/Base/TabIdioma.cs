...
{
    public class TabIdioma
    {
        public TABIDIOMA GetByKey(String key)
        {
            ...
            try
            {
                ...
                TABIDIOMA obj = (from c in ctx.TABIDIOMA where c.IDIOMA == key select c).FirstOrDefault();
        ...
        public TABIDIOMA ProcessarVazios(TABIDIOMA obj)
        {
            Type type = typeof(TABIDIOMA);
        ...
        public ResultList GetAll(Int32 pag, Int32 itemsByPag)
        {
            ...
            {
                var ctx = new DataGiicNetEntities();
                var obj = (from c in ctx.TABIDIOMA select c).ToList();
                if (obj.Count > 0)
                {
                    r.Status = true;
                    r.Lista = obj;
        ...
        public ResultList Insert(TABIDIOMA tab)
        {
            ...
            try
            {
                ...
                var ctx = new DataGiicNetEntities();
                TABIDIOMA obj = GetByKey(tab.IDIOMA);
                if (obj == null) ctx.TABIDIOMA.Add(tab);
        ...
        public ResultList Update(TABIDIOMA tab)
        {
            tab = ProcessarVazios(tab);
            var r = new ResultList();
            r.Status = false;
        ...
        public ResultList Valida(TABIDIOMA tab)
        {
            var r = new ResultList();
            r.Status = false;
            r.Erros = "";
