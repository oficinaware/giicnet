...
Public Class ServicosGcom
    Public Function ValidaPrePack(pnrenco As String, plnenco As Long) As Boolean
        ...
        If prepack.State = adStateOpen Then
            prepack.Close()
        End If
        prepack.Open("SELECT * FROM prepack WHERE nrenco = '" & pnrenco & "' AND lnenco = " & plnenco, cnn1, adOpenStatic, adLockReadOnly, adCmdText)
...
FIM:
    ...
    Public Function TotPACKCAB(PNRPACK As String)
        ...
        If Not PACKCAB.EOF Then
            ...
            Else
                ...
                With PACKCAB
                    !NRPARES = Nz(packlist_tot!QUANT, 0)
                    !NREMB = Nz(packlist_tot!NREMB, 0)
                    !PESOL = Nz(packlist_tot!PESOL, 0)
                    !PESOB = Nz(packlist_tot!PESOB, 0)
                    !VOLUME = Nz(packlist_tot!VOLEMB, 0)
                    .UpdateBatch()
                End With
            End If
    ...
    Public Function NrCartaoPrePackEnco(pnrenco As String) As Long
        ...
        If Not IsNull(RST!NRCARTAO) Then
            NrCartaoPrePackEnco = Nz(RST!NRCARTAO, 0) + 1
        End If
    ...
    Public Function NrCartaoPrePack() As Long
        ...
        pID_NRCARTAO = Nz(PARAM!ID_NRCARTAO, 0) + 1

        With PARAM
            If Nz(PARAM!ID_NRCARTAO_MAX, 0) = 0 Then
                !ID_NRCARTAO_MAX = 9999
            End If
            !ID_NRCARTAO = pID_NRCARTAO
            If pID_NRCARTAO >= Nz(PARAM!ID_NRCARTAO_MAX, 0) Then
                !ID_NRCARTAO = 0
            End If
            .UpdateBatch()
        End With
            ...
            Case 1
                ...
                Do While Not MODELOS.EOF
                    ...
                    If escalas.State = adStateOpen Then
                        escalas.Close()
                    End If
                    escalas.Open("select * from escalas where tipsort = '" & MODELOS.ESCALA & "'", cnn1, adOpenStatic, adLockReadOnly)
                    If Not escalas.EOF Then
                        For i = 1 To 28
                            If IsNull(escalas("T" & strzero(i, 2))).value Then
                                Exit For
                            End If
                            If Nz(MODELOS("P1" & strzero(i, 2)), 0) > 0 Then
                                ' calculo do novo preco
                                npreco = C_RND(Nz(MODELOS("P1" & strzero(i, 2)), 0) * PCOEF, 2)
                                npreco = C_RND(npreco, 0)
                                'npreco = npreco - 1
                                npreco = C_RND(npreco + Nz(pcent, 0) / 100, 2)

                                MODELOS("P3" & strzero(i, 2)).Value = npreco
                            End If
                        Next i
                        MODELOS.UpdateBatch()
                    End If
    ...
    Function CALC_DATA(DT1, dt2, DT3, DT4, DT5, DT6) As Date
        If Not IsNull(DT6) Then
            CALC_DATA = DT6
            GoTo FIM
        End If
        If Not IsNull(DT5) Then
            CALC_DATA = DT5
            GoTo FIM
        End If
        If Not IsNull(DT4) Then
            CALC_DATA = DT4
            GoTo FIM
        End If
        If Not IsNull(DT3) Then
            CALC_DATA = DT3
            GoTo FIM
        End If
        If Not IsNull(dt2) Then
            CALC_DATA = dt2
            GoTo FIM
        End If
        If Not IsNull(DT1) Then
            CALC_DATA = DT1
            GoTo FIM
        End If
FIM:
    ...
    Public Function APAGAR_ENC(pnrenco As String)
        ...
        ENCO.Open("SELECT * FROM enco WHERE nrenco = '" & pnrenco & "'", cnn1, adOpenStatic, adLockOptimistic, adCmdText)
        ...
        LNENCO.Open("SELECT * FROM lnenco WHERE lrenco = '" & pnrenco & "'", cnn1, adOpenStatic, adLockOptimistic, adCmdText)
        If Not LNENCO.EOF Then
            Do While LNENCO!LRENCO = pnrenco

                If ENCOPRC.State = adStateOpen Then
                    ENCOPRC.Close()
                End If

                ENCOPRC.Open("SELECT * FROM encoprc WHERE nrenco = '" & pnrenco & "' AND lnenco = " & LNENCO!LNENCO, cnn1, adOpenStatic, adLockOptimistic, adCmdText)
                If Not ENCOPRC.EOF Then
                    Do While ENCOPRC!NRENCO = LNENCO!LRENCO And ENCOPRC!LNENCO = ENCOPRC!LNENCO
                        ENCOPRC.Delete()
                        ENCOPRC.MoveNext()
                        If ENCOPRC.EOF Then
                            Exit Do
                ...
                ENCOSORT.Open("SELECT * FROM encosort WHERE nrenco = '" & LNENCO!LRENCO & "' AND lnenco = " & LNENCO!LNENCO, cnn1, adOpenStatic, adLockOptimistic, adCmdText)

                If Not ENCOSORT.EOF Then
                    Do While ENCOSORT!NRENCO = LNENCO!LRENCO And ENCOSORT!LNENCO = LNENCO!LNENCO
                        ENCOSORT.Delete()
                        ENCOSORT.MoveNext()
                ...
                Call APAGAR_NEC("EC", LNENCO!LRENCO, LNENCO!LNENCO)
                Call DEL_PREPACK(LNENCO!LRENCO, LNENCO!LNENCO)


                If RsPLN.State = adStateOpen Then
                    RsPLN.Close()
                End If
                RsPLN.Open("SELECT IDPLAN, COUNT(LRENCO) AS Nr_IdPlan FROM LNENCO GROUP BY IDPLAN HAVING IDPLAN = " & Nz(LNENCO!IDPLAN, 0), cnn1, adOpenStatic, adLockReadOnly)
                If Not RsPLN.EOF Then
                    If Nz(RsPLN!Nr_IdPlan, 0) = 1 Then
                        cnn1.Execute "DELETE FROM planning_ec WHERE ordfab = '" & strzero(Nz(RsPLN!IDPLAN, 0), 6) & "'"
                    End If
                End If
...
FIM:
    ...
    Public Function APAGAR_LENC(pnrenco As String, plnenco As Long)
        ...
        ENCOPRC.Open("SELECT * FROM encoprc WHERE nrenco = '" & pnrenco & "' AND lnenco = " & plnenco, cnn1, adOpenStatic, adLockOptimistic, adCmdText)

        If Not ENCOPRC.EOF Then
            Do While ENCOPRC!NRENCO = pnrenco And ENCOPRC!LNENCO = plnenco
                ENCOPRC.Delete()
                ENCOPRC.MoveNext()
                If ENCOPRC.EOF Then
                    Exit Do
        ...
        ENCOSORT.Open("SELECT * FROM encosort WHERE nrenco = '" & pnrenco & "' AND lnenco = " & plnenco, cnn1, adOpenStatic, adLockOptimistic, adCmdText)

        If Not ENCOSORT.EOF Then
            Do While ENCOSORT!NRENCO = pnrenco And ENCOSORT!LNENCO = plnenco
                ENCOSORT.Delete()
                ENCOSORT.MoveNext()
                If ENCOSORT.EOF Then
                    Exit Do
...
FIM:
    ...
    Function APAGAR_NEC(PCOD, pnrenco, plnenco)
        'Dim DBA As Database
        ...
        STKNEC.Open("SELECT * FROM stknec WHERE cod = '" & PCOD & "' and nrenco = '" & pnrenco & "' AND lnenco = " & plnenco, cnn1, adOpenStatic, adLockOptimistic, adCmdText)

        'stknec.Seek ">=", PCOD, pnrenco, plnenco
        If Not STKNEC.EOF Then
            Do While STKNEC!COD = PCOD And STKNEC!NRENCO = pnrenco And STKNEC!LNENCO = plnenco
...
FIM:
    ...
    Public Function DEL_PREPACK(pnrenco, plnenco)
        ...
        prepack.Open("SELECT * FROM prepack WHERE nrenco = '" & pnrenco & "' AND lnenco = " & plnenco, cnn1, adOpenStatic, adLockOptimistic, adCmdText)

        'prepack.Seek ">=", pnrenco, plnenco
        If Not prepack.EOF Then
            Do While prepack!NRENCO = pnrenco And prepack!LNENCO = plnenco
                prepack.Delete()
                prepack.MoveNext()
                If prepack.EOF Then
                    Exit Do
    ...
    Public Sub c_plafond(PCLIENTE)
        ...
        plafond_atrib = Nz(CLIENTES!PLAFOND, 0)
        ...
        If Not RSTenco.EOF Then
            PLAF(0) = C_RND(Nz(RSTenco!totvl, 0), 2)
        End If
...
FIM:
    ...
    Public Function NOVAENC(ptpdoc, ficheiro As String) As Object
        'Dim DB As Database
        Dim tabdoc As ADODB.Recordset, RST As ADODB.Recordset, MSG As Object
        Dim NOVONUM As Long, NUMANT As Long
        'Set DB = OpenDatabase(DIRECTORIO)
        'Set TABDOC = db.OpenRecordset("tabdoc")
        'TABDOC.Index = "primarykey"
        'Set rst = db.OpenRecordset(ficheiro)
